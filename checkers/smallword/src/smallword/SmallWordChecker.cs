	...
	internal class SmallWordChecker : IChecker
	{
		...
		public async Task Get(string host, PutResult put, string flag, int vuln)
		{
			...
			else
			{
				...
				if(!await DocxTraverse(result.Body, stream => ExtractTextFromXml(stream).ForEach(text => flagFound |= text.Contains(flag)), stream => imageFound |= ImageHelper.Equals(image, stream)))
